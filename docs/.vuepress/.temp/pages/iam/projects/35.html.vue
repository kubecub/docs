...
<h1 id="第35节-技术演进-上-虚拟化技术演进之路" tabindex="-1"><a class="header-anchor" href="#第35节-技术演进-上-虚拟化技术演进之路" aria-hidden="true">#</a> 第35节 技术演进（上）：虚拟化技术演进之路</h1>
<br>
...
<hr>
<nav class="table-of-contents"><ul><li><router-link to="#开始">开始</router-link></li><li><router-link to="#我们为什么使用云">我们为什么使用云？</router-link></li><li><router-link to="#虚拟化技术的演进">虚拟化技术的演进</router-link><ul><li><router-link to="#物理机阶段">物理机阶段</router-link></li><li><router-link to="#虚拟机阶段">虚拟机阶段</router-link></li><li><router-link to="#容器阶段">容器阶段</router-link></li><li><router-link to="#serverless阶段">Serverless阶段</router-link></li></ul></li><li><router-link to="#总结">总结</router-link></li><li><router-link to="#课后练习">课后练习</router-link></li><li><router-link to="#end-链接">END 链接</router-link></li></ul></nav>
<p>[TOC]</p>
...
<p>因为这一讲涉及的技术栈很多，所以我会把重点放在演进过程上，不会详细介绍每种技术的具体实现原理和使用方法。如果你感兴趣，可以自行学习，也可以参考我为你整理的这个资料：<a href="https://github.com/marmotedu/awesome-books#%E4%BA%91%E8%AE%A1%E7%AE%97" target="_blank" rel="noopener noreferrer">awesome-books<ExternalLinkIcon/></a>。</p>
...
<h2 id="虚拟化技术的演进" tabindex="-1"><a class="header-anchor" href="#虚拟化技术的演进" aria-hidden="true">#</a> 虚拟化技术的演进</h2>
...
<h3 id="物理机阶段" tabindex="-1"><a class="header-anchor" href="#物理机阶段" aria-hidden="true">#</a> 物理机阶段</h3>
...
<h4 id="虚拟化漏洞" tabindex="-1"><a class="header-anchor" href="#虚拟化漏洞" aria-hidden="true">#</a> 虚拟化漏洞</h4>
...
<p><a href="https://static001.geekbang.org/resource/image/6e/77/6ea013e621c58fb78d444449fb55f977.png?wh=1920x902" target="_blank" rel="noopener noreferrer"><img src="https://static001.geekbang.org/resource/image/6e/77/6ea013e621c58fb78d444449fb55f977.png?wh=1920x902" alt="图片"><ExternalLinkIcon/></a></p>
...
<h4 id="容器引擎docker" tabindex="-1"><a class="header-anchor" href="#容器引擎docker" aria-hidden="true">#</a> 容器引擎Docker</h4>
...
<h4 id="容器编排技术kubernetes" tabindex="-1"><a class="header-anchor" href="#容器编排技术kubernetes" aria-hidden="true">#</a> 容器编排技术Kubernetes</h4>
...
<ul>
<li>**Kube API Server：**提供了资源操作的唯一入口，并提供认证、授权、访问控制、API 注册和发现等机制。</li>
<li>**Kube Scheduler：**负责资源的调度，按照预定的调度策略将 Pod 调度到相应的机器上。</li>
<li>**Kube Controller Manager：**负责维护集群的状态，比如故障检测、自动扩展、滚动更新等。</li>
<li>**Cloud Controller Manager：**这个组件是在Kubernetes 1.6版本加入的与基础云提供商交互的控制器。</li>
<li>**Etcd：**分布式的K-V存储，独立于Kubernetes的开源组件。主要存储关键的元数据，支持水平扩容保障元数据的高可用性。基于Raft算法实现强一致性，独特的watch机制是Kubernetes设计的关键。</li>
</ul>
<p>介绍完了Master，再看看每一个Kubernetes Node需要有哪些组件。</p>
<ul>
<li>**Kubelet：**负责维持容器的生命周期，同时也负责 volume（CVI）和网络（CNI）的管理。</li>
<li>**kube-proxy：**kube-proxy是集群中每个节点上运行的网络代理，维护节点上的网络规则，它允许从集群的内部或外部网络与Pod进行网络通信，并负责为 Service 提供集群内部的服务发现和负载均衡。</li>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>在这个描述文件中，apiVersion和kind共同决定了当前YAML配置文件应该由谁来处理，前者表示描述文件使用的 API 组，后者表示一个 API 组中的一个资源类型。这里的 <code v-pre>v1</code> 和 <code v-pre>Pod</code> 表示的就是核心 API 组 <code v-pre>api/v1</code> 中的 <code v-pre>Pod</code> 类型对象。</p>
<p>metadata则是一些关于该对象的元数据，其中主要有<code v-pre>name</code>、<code v-pre>namespace</code>、<code v-pre>labels</code>、<code v-pre>annotations</code>。其中， <code v-pre>name</code> 需要在 <code v-pre>namespace</code> 下唯一，成为这个对象的唯一标识。<code v-pre>label</code>和<code v-pre>annotations</code>分别是这个对象的一些标签和一些注解，前者用于筛选，后者主要用来标注提示性的信息。</p>
<p>接下来，我再介绍下Pod、Deployment、Service 这3种对象。</p>
<ol>
<li>Pod</li>
</ol>
<p>Pod是Kubernetes中运行的最小的、最简单的计算单元，我觉得Pod也是Kubernetes最核心的对象。Pod中可以指定运行多个Containers，可以挂载volume来实现部署有状态的服务，这些都在<code v-pre>spec</code>中被指定。</p>
<p>对于任意类型的对象，<code v-pre>spec</code>都是用来描述开发人员或运维人员对这个对象所期望的状态的，对于不同类型的对象，<code v-pre>spec</code>有不同的子属性。</p>
...
<div class="language-yaml ext-yml line-numbers-mode"><pre v-pre class="language-yaml"><code><span class="token key atrule">apiVersion</span><span class="token punctuation">:</span> apps/v1 <span class="token comment"># for versions before 1.9.0 use apps/v1beta2</span>
...
<p>下面这个示例里，使用的是<code v-pre>run: my-nginx</code>这个label。这个Service绑定的就是上面那个Deployment部署的nginx服务器：</p>
<div class="language-yaml ext-yml line-numbers-mode"><pre v-pre class="language-yaml"><code><span class="token key atrule">apiVersion</span><span class="token punctuation">:</span> v1
...
<span class="token key atrule">spec</span><span class="token punctuation">:</span>
  ...
  <span class="token key atrule">selector</span><span class="token punctuation">:</span>
    <span class="token key atrule">run</span><span class="token punctuation">:</span> my<span class="token punctuation">-</span>nginx
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>最后我还想介绍下<strong>基于Kubernetes的容器云平台</strong>。各大公有云厂商，都有基于Kubernetes的容器管理平台，目前国内容器服务平台做得比较好的有<a href="https://cloud.tencent.com/document/product/457/6759" target="_blank" rel="noopener noreferrer">腾讯云容器服务TKE<ExternalLinkIcon/></a>、<a href="https://help.aliyun.com/product/85222.html" target="_blank" rel="noopener noreferrer">阿里云容器服务ACK<ExternalLinkIcon/></a>。</p>
...
<h3 id="serverless阶段" tabindex="-1"><a class="header-anchor" href="#serverless阶段" aria-hidden="true">#</a> Serverless阶段</h3>
...
<ul>
<li><a href="https://cloud.tencent.com/document/product/457/39804" target="_blank" rel="noopener noreferrer">EKS<ExternalLinkIcon/></a>：弹性容器服务（Elastic Kubernetes Service）是腾讯云容器服务推出的无需用户购买节点即可部署工作负载的服务模式。EKS 完全兼容原生 Kubernetes，支持使用原生方式购买及管理资源，按照容器真实使用的资源量计费。</li>
<li><a href="https://cloud.tencent.com/document/product/583" target="_blank" rel="noopener noreferrer">SCF<ExternalLinkIcon/></a>：云函数（Serverless Cloud Function）是腾讯云为企业和开发者们提供的无服务器执行环境，帮助你在无需购买和管理服务器的情况下运行代码。你只需使用平台支持的语言编写核心代码，并设置代码运行的条件，就能在腾讯云基础设施上弹性、安全地运行代码。</li>
<li><a href="https://cloud.tencent.com/document/product/1003/30488" target="_blank" rel="noopener noreferrer">TDSQL-C<ExternalLinkIcon/></a>：云原生数据库（Cloud Native Database TDSQL-C）是腾讯云自研的新一代高性能高可用的企业级分布式云数据库，具有高吞吐量、高可靠性等优点。</li>
</ul>
